...
public class Symbol() : SymbolBase(), ISymbol
{
    ...
    /// </summary>
    /// <summary>
    /// Deployed markets the scanner leaves out are the ones the user switched off in the settings
    /// (Settings.Products, the deployer being the product). Paragon ("para") starts switched off:
    /// it is not carried by Altrady, so every signal on one of its 25 instruments was a signal
    ...
    /// statement about the market, and since 05-09-2026 a choice the user makes per deployer. An
    /// instrument of a switched off deployer that is already in the database is deactivated by the
    /// caller like a delisted one, and its candles go with the next cleanup.
    /// </summary>
        ...
        SortedList<string, CryptoSymbol> activeSymbols)
    {
        ...
        foreach (var dex in dexList.Data)
        {
            ...
            // Switched off in the settings (see the remark above the method). Checked before the
            // request, so a deployer that is off costs no weight on the address either. Registering
            // the deployer here is also what puts it in the settings list, so the user can see and
            // switch every deployer HyperLiquid carries, not only the ones with instruments.
            if (!GlobalData.IsProductActive(dex.Name))
                continue;

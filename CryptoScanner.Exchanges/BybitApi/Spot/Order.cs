...
public class Order() : OrderBase(), IOrder
{
    ...
    public static void PickupOrder(CryptoSymbol symbol, CryptoOrder order, Bybit.Net.Objects.Models.V5.BybitOrder item)
    {
    ...
    public async Task<int> GetOrders(CryptoDatabase database, CryptoPosition position)
    {
        ...
        if (info.Success && info.Data != null)
        {
            ...
            foreach (var item in info.Data.List)
            {
                if (position.StepOrderList.ContainsKey(item.OrderId))
                {
                    ...
                    if (order != null)
                    {
                        ...
                        Order.PickupOrder(position.Symbol, order, item);
            ...
            GlobalData.AddErrorToLogTab($"Error reading orders from {ExchangeBase.ExchangeOptions.ExchangeName} for {position.Symbol.Name} {info.Error}");

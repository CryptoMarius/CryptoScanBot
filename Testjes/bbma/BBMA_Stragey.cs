...
namespace CryptoScanBot.TestStuff.bbma
{
    ...
    public class BBMA1
    {
        ...
        public void Analyze(Dictionary<TimeFrame, List<CryptoCandle>> multiTimeFrameCandles)
        {
            // ema50? 50 + lookback
            if (!multiTimeFrameCandles.ContainsKey(TimeFrame.H1) || multiTimeFrameCandles[TimeFrame.H1].Count < 50)
            {
                GlobalData.AddTextToLogTab("Insufficient data for H1. Need at least 50 candles.");
                return;
            }

            // Definieer TF-orde voor MTF-validatie (hoog naar laag: D1 > H4 > H1 > M15, etc.)
            var timeFrames = new[] { TimeFrame.D1, TimeFrame.H4, TimeFrame.H1, TimeFrame.M15, TimeFrame.M5 };
            // Per TF: signal-type -> (active, candles sinds signaal)
            var signals = new Dictionary<TimeFrame, Dictionary<string, (bool Active, int Age)>>(); 
            int maxSignalAge = 6; // Max leeftijd voor signalen (bijv. 12 H1-candles) (was 12)
            ...
            foreach (var tf in timeFrames)
            {
                ...
                for (int i = 50; i < candles.Count; i++)
                {
                    ...
                    foreach (var signal in signals[tf].ToList())
                    {
                        ...
                        // Deactiveer oude signalen
                        if (signal.Value.Age > maxSignalAge) 
                            signals[tf][signal.Key] = (false, signal.Value.Age); 
                    }
                    ...
                    if (recentExtremeSell)
                    {
                        ...
                        //GlobalData.AddTextToLogTab($"TPW (Take Profit Wajib) Suggestion after Extreme Sell at {candles[i].Date:yyyy-MM-dd HH:mm} - TP Level: {tpLevel:F4}");
                    ...
                    if (recentExtremeBuy)
                    {
                        ...
                        //GlobalData.AddTextToLogTab($"TPW (Take Profit Wajib) Suggestion after Extreme Buy at {candles[i].Date:yyyy-MM-dd HH:mm} - TP Level: {tpLevel:F4}");
                    ...
                    //    GlobalData.AddTextToLogTab($"{type} at {candles[i].Date:yyyy-MM-dd HH:mm} - Price: {candles[i].Close:F4}, LWMA5_low: {lwma5_low[i]:F4}");
                    //}
                    ...
                    //    GlobalData.AddTextToLogTab($"{type} at {candles[i].Date:yyyy-MM-dd HH:mm} - Price: {candles[i].Close:F4}, LWMA5_high: {lwma5_high[i]:F4}");
                    //}
                    //if (advanceExtremeBuy && isUptrend && (!signals[tf].ContainsKey("AdvanceExtremeBuy") || signals[tf]["AdvanceExtremeBuy"].Age <= maxSignalAge))
                    //{
                    //    GlobalData.AddTextToLogTab($"Advance Extreme Buy (EMA50 Wick Rejection) at {candles[i].Date:yyyy-MM-dd HH:mm} - Price: {candles[i].Close:F4}, EMA50: {ema50[i]:F4}");
                    //}
                    ...
                    //{
                    //    GlobalData.AddTextToLogTab($"Advance Extreme Sell (EMA50 Wick Rejection) at {candles[i].Date:yyyy-MM-dd HH:mm} - Price: {candles[i].Close:F4}, EMA50: {ema50[i]:F4}");
                    //}
                    //if (recentExtremeSell)
                    //{
                    //    decimal tpLevel = Math.Min(lwma5_high[i], lwma10_high[i]);
                    ...
                    //if (mlvPotential)
                    //{
                    //    GlobalData.AddTextToLogTab($"MLV (Market Loss Volume) at {candles[i].Date:yyyy-MM-dd HH:mm} - Price consolidating inside BB");
                    //}
                    //if (csdBull || earlyCsdBull)
                    //{
                    //    string type = earlyCsdBull ? "Early CSD (CSAK) Bull (High Risk)" : "CSD (CSAK) Bull (Direction Change Up)";
                    //    GlobalData.AddTextToLogTab($"{type} at {candles[i].Date:yyyy-MM-dd HH:mm}");
                    //}
                    //if (csdBear || earlyCsdBear)
                    //{
                    ...
                    //    GlobalData.AddTextToLogTab($"{type} at {candles[i].Date:yyyy-MM-dd HH:mm} - Strong bullish candle");
                    //}
                    //if (csmBear || earlyCsmBear)
                    //{
                    //    string type = earlyCsmBear ? "Early CSM Bear (High Risk)" : "CSM Bear (Momentum Down)";
                    //    GlobalData.AddTextToLogTab($"{type} at {candles[i].Date:yyyy-MM-dd HH:mm} - Strong bearish candle");
                    //}
                    ...
                    //    GlobalData.AddTextToLogTab($"Reentry Buy Zone (after CSD/CSM) at {candles[i].Date:yyyy-MM-dd HH:mm} - Zone: {zoneLow:F4} to {zoneHigh:F4}");
                    //}
                    ...
                    //    GlobalData.AddTextToLogTab($"Reentry Sell Zone (after CSD/CSM) at {candles[i].Date:yyyy-MM-dd HH:mm} - Zone: {zoneLow:F4} to {zoneHigh:F4}");
                    //}
                }
            }
            ...
            //GlobalData.AddTextToLogTab("Criteria: Codes like REM (Reentry on high TF + Extreme on middle + MLV on low). Only recent signals (age <= {maxSignalAge}).");
            ...
            for (int idx = 2; idx < timeFrames.Length; idx++) // Voor 3 TF's: high = D1, middle = H4, low = H1 or M15
            {
                ...
                foreach (var code in mtfCodes)
                {
                    ...
                    if (buyMatch)
                    {
                        GlobalData.AddTextToLogTab($"MTF Code {code.Code} Buy Confirmed on {lowTF} (High: {highTF} {code.HighSignalBuy}, Middle: {middleTF} {code.MiddleSignalBuy}, Low: {lowTF} {code.LowSignalBuy}) - Strong Reversal Setup!");
                    }
                    ...
                    if (sellMatch)
                    {
                        GlobalData.AddTextToLogTab($"MTF Code {code.Code} Sell Confirmed on {lowTF} (High: {highTF} {code.HighSignalSell}, Middle: {middleTF} {code.MiddleSignalSell}, Low: {lowTF} {code.LowSignalSell}) - Strong Reversal Setup!");
                    }
                }
            }
        }

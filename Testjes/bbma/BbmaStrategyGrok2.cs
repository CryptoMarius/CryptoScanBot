...
// Seem to work rather okay though

public class BbmaStrategyGrok2
{
    // Inputs
    public string MtfMode { get; set; } = "Both"; // TF1 only /Off
    ...
    public class AdminTimeFrame2
    {
        ...
        public int[] htf2ExtremeBuyBSHist = [];
        public int[] htf2ExtremeSellBSHist = [];
    ...
    void Part2(List<Quote> candles2, AdminTimeFrame2 tf2, AdminTimeFrame1 tf1)
    {
        ...
        tf2.htf2ExtremeBuyBSHist = Enumerable.Range(0, m).Select(j => GetSince(tf2.htf2ExtBuyBase, j, LookbackSig)).ToArray();
        tf2.htf2ExtremeSellBSHist = Enumerable.Range(0, m).Select(j => GetSince(tf2.htf2ExtSellBase, j, LookbackSig)).ToArray();
        ...
        for (int j = 0; j < m; j++)
        {
            ...
            tf2.htf2MhvBuyBase[j] = (candle.Low < (decimal)jLower && candle.Close > (decimal)jLower) && tf2.htf2ExtremeBuyBSHist[j] < LookbackSig;
            tf2.htf2MhvSellBase[j] = (candle.High > (decimal)jUpper && candle.Close < (decimal)jUpper) && tf2.htf2ExtremeSellBSHist[j] < LookbackSig;
        }

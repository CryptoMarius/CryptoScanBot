...
{
    public SignalSma50Sma20PriceLong(CryptoSymbol symbol, CryptoInterval interval, CryptoCandle candle) : base(symbol, interval, candle)
    {
    ...
    private static bool InLowerPartOfBollingerBands(CryptoCandle candle, decimal percentage)
    {
    ...
    public override bool IsSignal()
    {
        ...
        if (!InLowerPartOfBollingerBands(1, 7.5m))
            return false;
        ...
        if (Interval.IntervalPeriod < CryptoIntervalPeriod.interval8h)
        {
            ...
            if (!IndicatorsOkay(candle))
                return false;
            if (!InLowerPartOfBollingerBands(candle, 30m))
                return false;
        }

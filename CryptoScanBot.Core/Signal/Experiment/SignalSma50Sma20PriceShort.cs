...
public class SignalSma50Sma20PriceShort : SignalCreateBase
{
    public SignalSma50Sma20PriceShort(CryptoAccount account, CryptoSymbol symbol, CryptoInterval interval, CryptoCandle candle) : base(account, symbol, interval, candle)
    {
    ...
    private static bool IsInUpperPartOfBollingerBands(CryptoCandle candle, decimal percentage)
    {
        decimal value = (decimal)candle!.CandleData?.BollingerBandsUpperBand!;
    ...
    public override bool IsSignal()
    {
        ...
        if (!IsInUpperPartOfBollingerBands(1, 10))
            return false;
        ...
        if (Interval.IntervalPeriod < CryptoIntervalPeriod.interval8h)
        {
            ...
            if (!IndicatorsOkay(candle))
                return false;
            if (!IsInUpperPartOfBollingerBands(candle, 30))
                return false;
        }

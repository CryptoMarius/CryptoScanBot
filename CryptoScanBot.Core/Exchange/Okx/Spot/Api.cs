...
public class Api : ExchangeBase
{
    ...
    public override void ExchangeDefaults()
    {
        ...
        ExchangeOptions.ExchangeName = "Okx Spot";
        ...
        decimal quantity, decimal price, decimal? stop, decimal? limit, bool generateJsonDebug = false)
    {
        ...
        WebCallResult<OKXOrderPlaceResponse> result;
        switch (orderType)
        {
            case CryptoOrderType.Market:
                {
                    ...
                    result = await client.UnifiedApi.Trading.PlaceOrderAsync(position.Symbol.Name, side,
                        OrderType.Market, price * quantity); //, timeInForce: TimeInForce.GoodTillCanceled, isLeverage: false
                    if (!result.Success)
                    {
                    ...
                    if (result.Success && result.Data != null)
                    {
                        ...
                        tradeParams.OrderId = result.Data.OrderId.ToString();
                    }
            ...
            case CryptoOrderType.Limit:
                {
                    ...
                    result = await client.UnifiedApi.Trading.PlaceOrderAsync(position.Symbol.Name, side,
                    OrderType.Limit, quantity: quantity, price: price); //timeInForce: GoodTillCanceled, isLeverage: false
                    if (!result.Success)
                    {
                        tradeParams.Error = result.Error;
                        tradeParams.ResponseStatusCode = result.ResponseStatusCode;
    ...
    public override async Task<(bool succes, TradeParams? tradeParams)> Cancel(CryptoPosition position, CryptoPositionPart part, CryptoPositionStep step)
    {
        ...
        if (step.OrderId != null && step.OrderId != "")
        {
            ...
            var result = await client.UnifiedApi.Trading.CancelOrderAsync(position.Symbol.Name, long.Parse(step.OrderId)); 
            if (!result.Success)
            {
                tradeParams.Error = result.Error;
                tradeParams.ResponseStatusCode = result.ResponseStatusCode;

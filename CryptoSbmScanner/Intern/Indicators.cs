...
namespace CryptoSbmScanner
{
    ...
    public static class Indicators
    {
        ...
        public static List<CryptoBbResult> GetBollingerBands(List<CryptoCandle> history, int lookbackPeriods, decimal standardDeviations)
        {
            ...
            // initialize
            List<CryptoBbResult> results = new List<CryptoBbResult>(history.Count);

            // roll through quotes
            for (int i = 0; i < history.Count; i++)
            {
                CryptoBbResult result = new CryptoBbResult();
                result.Date = history[i].Date; // overkill, maar voor debug prima
                results.Add(result);
            ...
            decimal accelerationStep = 0.02m, decimal maxAccelerationFactor = 0.2m)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                ParabolicSarResult r = new ParabolicSarResult(candle.Date);
                results.Add(r);
        ...
        public static List<ParabolicSarResult> CalcParabolicSarJasonLam(List<CryptoCandle> history, decimal acceleration = 0.02m, decimal accelerationMax = 0.2m)
        {
            ...
            ParabolicSarResult r = new ParabolicSarResult(history[0].Date);
            r.Sar = (double)priorSar;
            r.IsReversal = null;
            results.Add(r);
        ...
        public static List<ParabolicSarResult> CalcParabolicSarTulip(List<CryptoCandle> history, decimal acceleration = 0.02m, decimal accelerationMax = 0.2m)
        {
            ...
            List<ParabolicSarResult> results = new List<ParabolicSarResult>(size);

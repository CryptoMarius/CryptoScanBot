...
public class ThreadCreateSignal
{
    ...
    private readonly AnalyseAlgoritmAlarmEvent AlarmEvent = null;
    ...
    public void Execute()
    {
        foreach (CryptoCandle candle in Queue.GetConsumingEnumerable(cancellationToken.Token))
        {
            try
            {
                ...
                {
                    if (candle.Symbol.IsSpotTradingAllowed && GlobalData.Settings.QuoteCoins.TryGetValue(candle.Symbol.Quote, out CryptoQuoteData quoteData) && quoteData.CreateSignals)
                    {
                        ...
                        foreach (CryptoInterval interval in GlobalData.IntervalList)
                        {
                            ...
                            if (GlobalData.Settings.Signal.AnalyseInterval[(int)interval.IntervalPeriod])
                            {
                                ...
                                if (nextCandleOpenTime % interval.Duration == 0)
                                {
                                    ...
                                    SignalCreate createSignal = new(candle.Symbol, interval, AlarmEvent);

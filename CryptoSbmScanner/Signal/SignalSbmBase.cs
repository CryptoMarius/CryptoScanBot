    ...
    public class SignalSbmBase : SignalBase
    {
...
#if DEBUG
        private string Toelichting(CryptoCandle candle)
        {
            string text = "";
        ...
        public bool hasCrossed200and50(int candleCount = 30)
        {
            ...
            while (candleCount >= 0)
            {
                ...
                if (Candles.TryGetValue(time, out lastCandle))
                {
        ...
        public bool IsMacdRecoveryOversold(int candleCount = 2)
        {
            ...
            while (candleCount > 0)
            {
                ...
                {
                    ExtraText = string.Format("De MACD[{0:N0}].Hist is ondertussen groen {0:N8}", iterator, last.CandleData.Macd.Histogram.Value);
                    return false;
                }
                iterator--;

                CryptoCandle prev;
                if (!Candles.TryGetValue(last.OpenTime - 1 * Interval.Duration, out prev))
                {
        ...
        public bool IsMacdRecoveryOverbought(int candleCount = 2)
        {
            ...
            while (candleCount > 0)
            {
                if (last.CandleData.Macd.Histogram.Value <= 0)
                {
                    ExtraText = string.Format("De MACD[{0:N0}].Hist is ondertussen rood {0:N8}", iterator, last.CandleData.Macd.Histogram.Value);
                    return false;
                }
                iterator--;

...
namespace CryptoSbmScanner
{
    ...
    public partial class FrmMain : Form //MetroFramework.Forms.MetroForm //Form //MaterialForm //
    {
        ...
        private readonly ColorSchemeTest theme = new ColorSchemeTest();
        ...
        private void CreateBarometerBitmap(CryptoQuoteData quoteData, CryptoInterval interval)
        {
            ...
            if (GlobalData.ExchangeListName.TryGetValue("Binance", out CryptoExchange exchange))
            {
                if ((quoteData != null) && (exchange.SymbolListName.TryGetValue(Constants.SymbolNameBarometerPrice + quoteData.Name, out CryptoSymbol symbol)))
                {
                    ...
                    {
                        // hoe breed?
                        Rectangle rect = new Rectangle(0, 0, 55, intHeight);
        ...
        private readonly List<SymbolHist> symbolHistList = new List<SymbolHist>(3);


        private static void ShowSymbolPrice(SymbolHist hist, ListViewItem item, CryptoExchange exchange,
            CryptoQuoteData quoteData, string baseCoin, SymbolValue tvValues, string caption, string valueText)
        {
        ...
        private void ListViewSymbolPrices_DoubleClick(object sender, EventArgs e)
        {
            if (listViewSymbolPrices.SelectedItems.Count > 0)
            {
                ...
                else if (col < 7)
                {
                    ...
                    SymbolValue tvValues = (SymbolValue)subItem.Tag;
                    if (tvValues == null)
                        return;

                    if (tvValues.Url != null && tvValues.Url != "")
                    {
        ...
        private async void TimerShowBarometer_Tick(object sender, EventArgs e)
        {
            if (GlobalData.ApplicationStatus != ApplicationStatus.AppStatusExiting)
            {
                    ...
                    if (GlobalData.FearAndGreedIndex.LastCheck == null || DateTime.UtcNow >= GlobalData.FearAndGreedIndex.LastCheck)
                    {
                        ...
                        //if (response.IsSuccessStatusCode)
                        {
                            ...
                            using (System.IO.StreamReader sr = new System.IO.StreamReader(data))
                            {
                                ...
                                GlobalData.FearAndGreedIndex.Lp = decimal.Parse(jsonData["data"][0]["value"].Value<string>());
                ...
                {
                    GlobalData.FearAndGreedIndex.Lp = 0;
                    GlobalData.FearAndGreedIndex.LastCheck = DateTime.UtcNow.AddMinutes(5); // = Next check
                }
        ...
        private readonly Queue<CryptoSignal> signalQueue = new Queue<CryptoSignal>();

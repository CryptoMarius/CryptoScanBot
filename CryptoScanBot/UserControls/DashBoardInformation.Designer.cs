namespace CryptoScanBot.TradingView;

partial class DashBoardInformation
{
    /// <summary> 
    /// Required designer variable.
    /// </summary>
    private System.ComponentModel.IContainer components = null;

    /// <summary> 
    /// Clean up any resources being used.
    /// </summary>
    /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
    protected override void Dispose(bool disposing)
    {
        if (disposing && (components != null))
        {
            components.Dispose();
        }
        base.Dispose(disposing);
    }

    #region Component Designer generated code

    /// <summary> 
    /// Required method for Designer support - do not modify 
    /// the contents of this method with the code editor.
    /// </summary>
    private void InitializeComponent()
    {
        label31 = new Label();
        label32 = new Label();
        label33 = new Label();
        label34 = new Label();
        label35 = new Label();
        panelLeft = new Panel();
        pictureBoxTrader = new PictureBox();
        pictureBoxScanner = new PictureBox();
        labelAppicationStatus = new Label();
        labelBm4h = new Label();
        labelBm1d = new Label();
        labelBmTime = new Label();
        labelBm1h = new Label();
        labelBarometerDateValue = new Label();
        EditBarometerInterval = new ComboBox();
        EditBarometerQuote = new ComboBox();
        pictureBoxRulez = new PictureBox();
        pictureBox1 = new PictureBox();
        panel1 = new Panel();
        label30 = new Label();
        label29 = new Label();
        label28 = new Label();
        label27 = new Label();
        label26 = new Label();
        label25 = new Label();
        label24 = new Label();
        label23 = new Label();
        label22 = new Label();
        label21 = new Label();
        label20 = new Label();
        label19 = new Label();
        label18 = new Label();
        label17 = new Label();
        label16 = new Label();
        label15 = new Label();
        label14 = new Label();
        label13 = new Label();
        label12 = new Label();
        labelSymbol1Volume = new Label();
        label10 = new Label();
        label9 = new Label();
        label8 = new Label();
        label7 = new Label();
        labelSymbol1Price = new Label();
        labelSymbol5Name = new Label();
        labelSymbol4Name = new Label();
        labelSymbol3Name = new Label();
        labelSymbol2Name = new Label();
        labelSymbol1Name = new Label();
        panelLeft.SuspendLayout();
        ((System.ComponentModel.ISupportInitialize)pictureBoxTrader).BeginInit();
        ((System.ComponentModel.ISupportInitialize)pictureBoxScanner).BeginInit();
        ((System.ComponentModel.ISupportInitialize)pictureBoxRulez).BeginInit();
        ((System.ComponentModel.ISupportInitialize)pictureBox1).BeginInit();
        panel1.SuspendLayout();
        SuspendLayout();
        // 
        // label31
        // 
        label31.Location = new Point(640, 4);
        label31.Name = "label31";
        label31.Size = new Size(60, 15);
        label31.TabIndex = 109;
        label31.Text = "label31";
        label31.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label32
        // 
        label32.Location = new Point(640, 24);
        label32.Name = "label32";
        label32.Size = new Size(60, 15);
        label32.TabIndex = 110;
        label32.Text = "label32";
        label32.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label33
        // 
        label33.Location = new Point(640, 44);
        label33.Name = "label33";
        label33.Size = new Size(60, 15);
        label33.TabIndex = 111;
        label33.Text = "label33";
        label33.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label34
        // 
        label34.Location = new Point(640, 64);
        label34.Name = "label34";
        label34.Size = new Size(60, 15);
        label34.TabIndex = 112;
        label34.Text = "label34";
        label34.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label35
        // 
        label35.Location = new Point(640, 84);
        label35.Name = "label35";
        label35.Size = new Size(60, 15);
        label35.TabIndex = 113;
        label35.Text = "label35";
        label35.TextAlign = ContentAlignment.MiddleRight;
        // 
        // panelLeft
        // 
        panelLeft.AutoSize = true;
        panelLeft.Controls.Add(pictureBoxTrader);
        panelLeft.Controls.Add(pictureBoxScanner);
        panelLeft.Controls.Add(labelAppicationStatus);
        panelLeft.Controls.Add(labelBm4h);
        panelLeft.Controls.Add(labelBm1d);
        panelLeft.Controls.Add(labelBmTime);
        panelLeft.Controls.Add(labelBm1h);
        panelLeft.Controls.Add(labelBarometerDateValue);
        panelLeft.Controls.Add(EditBarometerInterval);
        panelLeft.Controls.Add(EditBarometerQuote);
        panelLeft.Controls.Add(pictureBoxRulez);
        panelLeft.Dock = DockStyle.Left;
        panelLeft.Location = new Point(0, 0);
        panelLeft.Name = "panelLeft";
        panelLeft.Size = new Size(169, 103);
        panelLeft.TabIndex = 132;
        // 
        // pictureBoxTrader
        // 
        pictureBoxTrader.Location = new Point(3, 37);
        pictureBoxTrader.Name = "pictureBoxTrader";
        pictureBoxTrader.Size = new Size(25, 26);
        pictureBoxTrader.TabIndex = 130;
        pictureBoxTrader.TabStop = false;
        // 
        // pictureBoxScanner
        // 
        pictureBoxScanner.Location = new Point(3, 6);
        pictureBoxScanner.Name = "pictureBoxScanner";
        pictureBoxScanner.Size = new Size(27, 23);
        pictureBoxScanner.TabIndex = 129;
        pictureBoxScanner.TabStop = false;
        // 
        // labelAppicationStatus
        // 
        labelAppicationStatus.AutoSize = true;
        labelAppicationStatus.Location = new Point(35, 82);
        labelAppicationStatus.Margin = new Padding(4, 0, 4, 0);
        labelAppicationStatus.Name = "labelAppicationStatus";
        labelAppicationStatus.Size = new Size(62, 15);
        labelAppicationStatus.TabIndex = 128;
        labelAppicationStatus.Text = "Barometer";
        // 
        // labelBm4h
        // 
        labelBm4h.Anchor = AnchorStyles.Top | AnchorStyles.Right;
        labelBm4h.Location = new Point(111, 25);
        labelBm4h.Margin = new Padding(4, 0, 4, 0);
        labelBm4h.Name = "labelBm4h";
        labelBm4h.Size = new Size(51, 15);
        labelBm4h.TabIndex = 126;
        labelBm4h.Text = "4h";
        labelBm4h.TextAlign = ContentAlignment.MiddleRight;
        // 
        // labelBm1d
        // 
        labelBm1d.Anchor = AnchorStyles.Top | AnchorStyles.Right;
        labelBm1d.Location = new Point(111, 45);
        labelBm1d.Margin = new Padding(4, 0, 4, 0);
        labelBm1d.Name = "labelBm1d";
        labelBm1d.Size = new Size(51, 15);
        labelBm1d.TabIndex = 125;
        labelBm1d.Text = "1d";
        labelBm1d.TextAlign = ContentAlignment.MiddleRight;
        // 
        // labelBmTime
        // 
        labelBmTime.Anchor = AnchorStyles.Top | AnchorStyles.Right;
        labelBmTime.AutoSize = true;
        labelBmTime.Location = new Point(132, 70);
        labelBmTime.Margin = new Padding(4, 0, 4, 0);
        labelBmTime.Name = "labelBmTime";
        labelBmTime.Size = new Size(31, 15);
        labelBmTime.TabIndex = 124;
        labelBmTime.Text = "time";
        labelBmTime.TextAlign = ContentAlignment.MiddleCenter;
        // 
        // labelBm1h
        // 
        labelBm1h.Anchor = AnchorStyles.Top | AnchorStyles.Right;
        labelBm1h.Location = new Point(111, 4);
        labelBm1h.Margin = new Padding(4, 0, 4, 0);
        labelBm1h.Name = "labelBm1h";
        labelBm1h.Size = new Size(51, 15);
        labelBm1h.TabIndex = 123;
        labelBm1h.Text = "1h";
        labelBm1h.TextAlign = ContentAlignment.MiddleRight;
        // 
        // labelBarometerDateValue
        // 
        labelBarometerDateValue.AutoSize = true;
        labelBarometerDateValue.Location = new Point(35, 63);
        labelBarometerDateValue.Margin = new Padding(4, 0, 4, 0);
        labelBarometerDateValue.Name = "labelBarometerDateValue";
        labelBarometerDateValue.Size = new Size(62, 15);
        labelBarometerDateValue.TabIndex = 74;
        labelBarometerDateValue.Text = "Barometer";
        // 
        // EditBarometerInterval
        // 
        EditBarometerInterval.DropDownStyle = ComboBoxStyle.DropDownList;
        EditBarometerInterval.FormattingEnabled = true;
        EditBarometerInterval.Location = new Point(35, 34);
        EditBarometerInterval.Margin = new Padding(2);
        EditBarometerInterval.Name = "EditBarometerInterval";
        EditBarometerInterval.Size = new Size(70, 23);
        EditBarometerInterval.TabIndex = 73;
        // 
        // EditBarometerQuote
        // 
        EditBarometerQuote.DropDownStyle = ComboBoxStyle.DropDownList;
        EditBarometerQuote.FormattingEnabled = true;
        EditBarometerQuote.Location = new Point(35, 6);
        EditBarometerQuote.Margin = new Padding(2);
        EditBarometerQuote.Name = "EditBarometerQuote";
        EditBarometerQuote.Size = new Size(70, 23);
        EditBarometerQuote.TabIndex = 71;
        // 
        // pictureBoxRulez
        // 
        pictureBoxRulez.Location = new Point(3, 70);
        pictureBoxRulez.Name = "pictureBoxRulez";
        pictureBoxRulez.Size = new Size(25, 25);
        pictureBoxRulez.TabIndex = 131;
        pictureBoxRulez.TabStop = false;
        // 
        // pictureBox1
        // 
        pictureBox1.Dock = DockStyle.Left;
        pictureBox1.Location = new Point(169, 0);
        pictureBox1.Name = "pictureBox1";
        pictureBox1.Size = new Size(403, 103);
        pictureBox1.TabIndex = 133;
        pictureBox1.TabStop = false;
        // 
        // panel1
        // 
        panel1.AutoSize = true;
        panel1.Controls.Add(label30);
        panel1.Controls.Add(label29);
        panel1.Controls.Add(label28);
        panel1.Controls.Add(label27);
        panel1.Controls.Add(label26);
        panel1.Controls.Add(label25);
        panel1.Controls.Add(label24);
        panel1.Controls.Add(label23);
        panel1.Controls.Add(label22);
        panel1.Controls.Add(label21);
        panel1.Controls.Add(label20);
        panel1.Controls.Add(label19);
        panel1.Controls.Add(label18);
        panel1.Controls.Add(label17);
        panel1.Controls.Add(label16);
        panel1.Controls.Add(label15);
        panel1.Controls.Add(label14);
        panel1.Controls.Add(label13);
        panel1.Controls.Add(label12);
        panel1.Controls.Add(labelSymbol1Volume);
        panel1.Controls.Add(label10);
        panel1.Controls.Add(label9);
        panel1.Controls.Add(label8);
        panel1.Controls.Add(label7);
        panel1.Controls.Add(labelSymbol1Price);
        panel1.Controls.Add(labelSymbol5Name);
        panel1.Controls.Add(labelSymbol4Name);
        panel1.Controls.Add(labelSymbol3Name);
        panel1.Controls.Add(labelSymbol2Name);
        panel1.Controls.Add(labelSymbol1Name);
        panel1.Controls.Add(label35);
        panel1.Controls.Add(label34);
        panel1.Controls.Add(label33);
        panel1.Controls.Add(label32);
        panel1.Controls.Add(label31);
        panel1.Dock = DockStyle.Fill;
        panel1.Location = new Point(572, 0);
        panel1.Name = "panel1";
        panel1.Size = new Size(1065, 103);
        panel1.TabIndex = 134;
        // 
        // label30
        // 
        label30.AutoSize = true;
        label30.Location = new Point(515, 84);
        label30.Name = "label30";
        label30.Size = new Size(44, 15);
        label30.TabIndex = 138;
        label30.Text = "label30";
        // 
        // label29
        // 
        label29.AutoSize = true;
        label29.Location = new Point(515, 64);
        label29.Name = "label29";
        label29.Size = new Size(44, 15);
        label29.TabIndex = 137;
        label29.Text = "label29";
        // 
        // label28
        // 
        label28.AutoSize = true;
        label28.Location = new Point(515, 43);
        label28.Name = "label28";
        label28.Size = new Size(44, 15);
        label28.TabIndex = 136;
        label28.Text = "label28";
        // 
        // label27
        // 
        label27.AutoSize = true;
        label27.Location = new Point(515, 24);
        label27.Name = "label27";
        label27.Size = new Size(44, 15);
        label27.TabIndex = 135;
        label27.Text = "label27";
        // 
        // label26
        // 
        label26.AutoSize = true;
        label26.Location = new Point(515, 4);
        label26.Name = "label26";
        label26.Size = new Size(44, 15);
        label26.TabIndex = 134;
        label26.Text = "label26";
        // 
        // label25
        // 
        label25.Location = new Point(395, 84);
        label25.Name = "label25";
        label25.Size = new Size(95, 15);
        label25.TabIndex = 133;
        label25.Text = "label25";
        label25.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label24
        // 
        label24.Location = new Point(395, 64);
        label24.Name = "label24";
        label24.Size = new Size(95, 15);
        label24.TabIndex = 132;
        label24.Text = "label24";
        label24.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label23
        // 
        label23.Location = new Point(395, 44);
        label23.Name = "label23";
        label23.Size = new Size(95, 15);
        label23.TabIndex = 131;
        label23.Text = "label23";
        label23.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label22
        // 
        label22.Location = new Point(395, 24);
        label22.Name = "label22";
        label22.Size = new Size(95, 15);
        label22.TabIndex = 130;
        label22.Text = "label22";
        label22.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label21
        // 
        label21.Location = new Point(395, 4);
        label21.Name = "label21";
        label21.Size = new Size(95, 15);
        label21.TabIndex = 129;
        label21.Text = "label21";
        label21.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label20
        // 
        label20.AutoSize = true;
        label20.Location = new Point(269, 84);
        label20.Name = "label20";
        label20.Size = new Size(44, 15);
        label20.TabIndex = 128;
        label20.Text = "label20";
        // 
        // label19
        // 
        label19.AutoSize = true;
        label19.Location = new Point(269, 64);
        label19.Name = "label19";
        label19.Size = new Size(44, 15);
        label19.TabIndex = 127;
        label19.Text = "label19";
        // 
        // label18
        // 
        label18.AutoSize = true;
        label18.Location = new Point(269, 44);
        label18.Name = "label18";
        label18.Size = new Size(44, 15);
        label18.TabIndex = 126;
        label18.Text = "label18";
        // 
        // label17
        // 
        label17.AutoSize = true;
        label17.Location = new Point(269, 24);
        label17.Name = "label17";
        label17.Size = new Size(44, 15);
        label17.TabIndex = 125;
        label17.Text = "label17";
        // 
        // label16
        // 
        label16.AutoSize = true;
        label16.Location = new Point(269, 4);
        label16.Name = "label16";
        label16.Size = new Size(44, 15);
        label16.TabIndex = 124;
        label16.Text = "label16";
        // 
        // label15
        // 
        label15.Location = new Point(159, 84);
        label15.Name = "label15";
        label15.Size = new Size(75, 15);
        label15.TabIndex = 123;
        label15.Text = "label15";
        label15.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label14
        // 
        label14.Location = new Point(159, 64);
        label14.Name = "label14";
        label14.Size = new Size(75, 15);
        label14.TabIndex = 122;
        label14.Text = "label14";
        label14.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label13
        // 
        label13.Location = new Point(159, 44);
        label13.Name = "label13";
        label13.Size = new Size(75, 15);
        label13.TabIndex = 121;
        label13.Text = "label13";
        label13.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label12
        // 
        label12.Location = new Point(159, 24);
        label12.Name = "label12";
        label12.Size = new Size(75, 15);
        label12.TabIndex = 120;
        label12.Text = "label12";
        label12.TextAlign = ContentAlignment.MiddleRight;
        // 
        // labelSymbol1Volume
        // 
        labelSymbol1Volume.Location = new Point(159, 4);
        labelSymbol1Volume.Name = "labelSymbol1Volume";
        labelSymbol1Volume.Size = new Size(75, 15);
        labelSymbol1Volume.TabIndex = 119;
        labelSymbol1Volume.Text = "label11";
        labelSymbol1Volume.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label10
        // 
        label10.Location = new Point(78, 84);
        label10.Name = "label10";
        label10.Size = new Size(60, 15);
        label10.TabIndex = 118;
        label10.Text = "label10";
        label10.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label9
        // 
        label9.Location = new Point(78, 64);
        label9.Name = "label9";
        label9.Size = new Size(60, 15);
        label9.TabIndex = 117;
        label9.Text = "label9";
        label9.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label8
        // 
        label8.Location = new Point(78, 44);
        label8.Name = "label8";
        label8.Size = new Size(60, 15);
        label8.TabIndex = 116;
        label8.Text = "label8";
        label8.TextAlign = ContentAlignment.MiddleRight;
        // 
        // label7
        // 
        label7.Location = new Point(78, 24);
        label7.Name = "label7";
        label7.Size = new Size(60, 15);
        label7.TabIndex = 115;
        label7.Text = "label7";
        label7.TextAlign = ContentAlignment.MiddleRight;
        // 
        // labelSymbol1Price
        // 
        labelSymbol1Price.Location = new Point(78, 4);
        labelSymbol1Price.Name = "labelSymbol1Price";
        labelSymbol1Price.Size = new Size(60, 15);
        labelSymbol1Price.TabIndex = 114;
        labelSymbol1Price.Text = "label6";
        labelSymbol1Price.TextAlign = ContentAlignment.MiddleRight;
        // 
        // labelSymbol5Name
        // 
        labelSymbol5Name.AutoSize = true;
        labelSymbol5Name.Location = new Point(6, 84);
        labelSymbol5Name.Name = "labelSymbol5Name";
        labelSymbol5Name.Size = new Size(38, 15);
        labelSymbol5Name.TabIndex = 113;
        labelSymbol5Name.Text = "label5";
        // 
        // labelSymbol4Name
        // 
        labelSymbol4Name.AutoSize = true;
        labelSymbol4Name.Location = new Point(6, 64);
        labelSymbol4Name.Name = "labelSymbol4Name";
        labelSymbol4Name.Size = new Size(38, 15);
        labelSymbol4Name.TabIndex = 112;
        labelSymbol4Name.Text = "label4";
        // 
        // labelSymbol3Name
        // 
        labelSymbol3Name.AutoSize = true;
        labelSymbol3Name.Location = new Point(6, 44);
        labelSymbol3Name.Name = "labelSymbol3Name";
        labelSymbol3Name.Size = new Size(38, 15);
        labelSymbol3Name.TabIndex = 111;
        labelSymbol3Name.Text = "label3";
        // 
        // labelSymbol2Name
        // 
        labelSymbol2Name.AutoSize = true;
        labelSymbol2Name.Location = new Point(6, 24);
        labelSymbol2Name.Name = "labelSymbol2Name";
        labelSymbol2Name.Size = new Size(38, 15);
        labelSymbol2Name.TabIndex = 110;
        labelSymbol2Name.Text = "label2";
        // 
        // labelSymbol1Name
        // 
        labelSymbol1Name.AutoSize = true;
        labelSymbol1Name.Location = new Point(6, 4);
        labelSymbol1Name.Name = "labelSymbol1Name";
        labelSymbol1Name.Size = new Size(38, 15);
        labelSymbol1Name.TabIndex = 109;
        labelSymbol1Name.Text = "label1";
        // 
        // DashBoardInformation
        // 
        AutoScaleDimensions = new SizeF(7F, 15F);
        AutoScaleMode = AutoScaleMode.Font;
        Controls.Add(panel1);
        Controls.Add(pictureBox1);
        Controls.Add(panelLeft);
        Name = "DashBoardInformation";
        Size = new Size(1637, 103);
        panelLeft.ResumeLayout(false);
        panelLeft.PerformLayout();
        ((System.ComponentModel.ISupportInitialize)pictureBoxTrader).EndInit();
        ((System.ComponentModel.ISupportInitialize)pictureBoxScanner).EndInit();
        ((System.ComponentModel.ISupportInitialize)pictureBoxRulez).EndInit();
        ((System.ComponentModel.ISupportInitialize)pictureBox1).EndInit();
        panel1.ResumeLayout(false);
        panel1.PerformLayout();
        ResumeLayout(false);
        PerformLayout();
    }

    #endregion
    private Label label31;
    private Label label32;
    private Label label33;
    private Label label34;
    private Label label35;
    private Panel panelLeft;
    private ComboBox EditBarometerInterval;
    private PictureBox pictureBoxTrader;
    private PictureBox pictureBoxRulez;
    private Label labelAppicationStatus;
    private Label labelBarometerDateValue;
    private Label labelBmTime;
    private Label labelBm1d;
    private Label labelBm4h;
    private Label labelBm1h;
    private ComboBox EditBarometerQuote;
    private PictureBox pictureBoxScanner;
    private PictureBox pictureBox1;
    private Panel panel1;
    private Label label30;
    private Label label29;
    private Label label28;
    private Label label27;
    private Label label26;
    private Label label25;
    private Label label24;
    private Label label23;
    private Label label22;
    private Label label21;
    private Label label20;
    private Label label19;
    private Label label18;
    private Label label17;
    private Label label16;
    private Label label15;
    private Label label14;
    private Label label13;
    private Label label12;
    private Label labelSymbol1Volume;
    private Label label10;
    private Label label9;
    private Label label8;
    private Label label7;
    private Label labelSymbol1Price;
    private Label labelSymbol5Name;
    private Label labelSymbol4Name;
    private Label labelSymbol3Name;
    private Label labelSymbol2Name;
    private Label labelSymbol1Name;
}

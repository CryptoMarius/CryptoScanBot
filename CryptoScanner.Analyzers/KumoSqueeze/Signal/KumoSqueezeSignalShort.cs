...
public class KumoSqueezeSignalShort : KumoSqueezeSignalBase
{
    public override bool IsSignal()
    {
        ...
        // 3. Price closes below the lower Bollinger Band
        if (!CandleLast.IsBelowBollingerBands(useLowHigh: false))
        {
            ExtraText = "Close not below lower BB";
            return false;
        }
        ...
        // 6. Price must be below the cloud (close < min(Senkou A, Senkou B))
        decimal cloudBottom = Math.Min((decimal)cloud.SenkouSpanA!, (decimal)cloud.SenkouSpanB!);
        if (CandleLast.Candle.Close >= cloudBottom)
        {
            ExtraText = $"Close {CandleLast.Candle.Close} not below Kumo bottom {cloudBottom:N8}";
            return false;
        }

        // 7. Cloud is bearish or neutral (Senkou A <= Senkou B)
        if (cloud.SenkouSpanA > cloud.SenkouSpanB)
        {
            ExtraText = "Cloud is bullish (Span A > Span B)";
            return false;
        }
        ...
        {
            ExtraText = $"RSI {CandleLast.CandleData.Rsi:N2} not below 50";
            return false;
        }
        ...
        ExtraText = $"Kumo squeeze breakout SHORT (bb={CandleLast.CandleData.BollingerBandsPercentage:N2}%, " +
                    $"rsi={CandleLast.CandleData.Rsi:N2}, cloud.bottom={cloudBottom:N8})";
        return true;
    }

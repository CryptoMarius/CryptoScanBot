...
namespace CryptoScanner.Analyzers.TradingBuddy.Tests;

/// <summary>
/// Parity tests for the scanner's VBS band construction (<see cref="VbsBandsHelper.ComputeBands"/>)
/// against the bands served by the TradingBuddy backend (scanner3.tradingbuddy.io
/// /api/v1/baba/bands). The fixtures under TradingBuddy/Tests/Fixtures hold, per symbol, the exact
...
/// </summary>
[TestClass]
public class VbsBandsParityTests
{
    ...
    // Symbols that have a fixture under TradingBuddy/Tests/Fixtures.
    ...
    public static void ClassInit(TestContext _)
    {
        ...
        // CreateSettings() replaces the plugin's static Settings with a fresh VbsSettings.
        VbsPlugin.CreateSettings();
    }
    ...
    {
        string path = Path.Combine(AppContext.BaseDirectory, "Signal", "TradingBuddy", "Fixtures", $"{symbol}-1h.json");
        // Fixtures are linked into CoreTests under Signal\TradingBuddy\Fixtures via csproj
    ...
    public void ScannerBands_MatchTradingBuddy(string symbol)
    {
        ...
        VbsBandsHelper.BandValue[] bands = VbsBandsHelper.ComputeBands(candles);
    ...
    public void Basis_EqualsVolumeWeightedVwmaOfHlc3()
    {
        ...
        List<CryptoCandle> candles = BuildCandles(fx);
        int len = VbsPlugin.Settings.Length;
    ...
    public void Bands_AreSymmetricAroundBasis_AndUseMultTimesVwStdev()
    {
        ...
        List<CryptoCandle> candles = BuildCandles(fx);
        double mult = VbsPlugin.Settings.Mult;

        VbsBandsHelper.BandValue[] bands = VbsBandsHelper.ComputeBands(candles);

        foreach (VbsBandsHelper.BandValue b in bands)
        {
            if (!b.HasValue)
                continue;

...
public class SignalEma9CrossedKeltnerCenter : SignalCreateBase
{
    public SignalEma9CrossedKeltnerCenter(CryptoSymbol symbol, CryptoInterval interval, CryptoCandle candle) : base(symbol, interval, candle)
    {
    }
    ...
    public override bool IsSignal()
    {
        ...
        if (GetPrevCandle(CandleLast, out CryptoCandle? candlePrev))
        {
            ...
            {
                if (candlePrev!.CandleData!.Ema9 > candlePrev.CandleData.KeltnerCenterLine
                    && CandleLast.CandleData!.Ema9 <= CandleLast.CandleData.KeltnerCenterLine)
                {
                    return true;
                }
            }

...
/// <summary>
/// Box Theory — Short entry.
///
/// The "box" is the high/low range of the previous full 1d candle. A short signal fires
/// the moment the active-interval close breaks below the previous-day low.
///
/// We require the previous active-interval close to still have been at or above the box
/// bottom, so the signal fires once on the transition (not on every candle that happens
/// to stay below the level for the rest of the day).
/// </summary>
public class SignalBoxShort : SignalCreateBase
{
    ...
    public override bool IsSignal()
    {
        ...
        if (lastClose >= boxBottom)
        {
            ExtraText = $"waiting for break below prev-day low {boxBottom:N8}";
            return false;
        }
        ...
        if (prev.Candle.Close < boxBottom)
        {
            ExtraText = "prev-day low was already broken in previous candle";
            return false;
        }

        ExtraText = $"Box Short break @ {lastClose:N8} (prev day {boxBottom:N8}-{boxTop:N8})";
        return true;
    }

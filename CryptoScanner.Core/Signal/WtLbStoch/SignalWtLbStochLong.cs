...
/// <summary>
/// Long variant of the wtlb.stoch strategy.
///
/// Setup (in evaluation order — cheapest first):
///   1. BB width inside the Stobb-configured range.
///   2. Optional trend filter: close > SMA200.
///   3. Stoch %K must have crossed up through StochCenterLevel within the last
///      StochCrossLookback candles.
///   4. WT1 crosses up through WtCrossLongLevel (e.g. -40) between the previous and the
///      current candle — the recovery from the deep oversold extreme.
///   5. Qualifier: WT1 must have been uninterruptedly below 0 for at least
///      WtConsecutiveBarsBelowAboveZero bars ending at the bar before the cross.
/// </summary>
public class SignalWtLbStochLong : SignalWtLbStochBase
{
    public override bool IsSignal()
    {
        ExtraText = "";

        // 1. BB width — reuse Stobb thresholds (same convention as wt.lb).
        ...
        if (!stochCrossed)
        {
            ExtraText = $"stoch %K did not cross up through {centerLevel:N0} in last {settings.StochCrossLookback}";
            return false;
        }
        ...
        double crossLevel = (double)settings.WtCrossLongLevel;
        if (!(prev1.Wt1.Value <= crossLevel && curr.Wt1.Value > crossLevel))
        {
            ExtraText = $"no wt1 cross up through {crossLevel:N1}";
            return false;
        }

        // 5. Qualifier — wt1 uninterruptedly below 0 for at least N bars, ending at prev1.
        int consecutive = 0;
        ...
        {
            ExtraText = $"only {consecutive} consecutive bars below 0 (need {settings.WtConsecutiveBarsBelowAboveZero})";
            return false;
        }

        ExtraText = $"wt1 cross up {crossLevel:N1} after {consecutive} bars below 0, " +
